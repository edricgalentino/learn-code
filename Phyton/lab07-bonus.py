...
4. TUJUAN_KELAS_TERMURAH <tujuan_akhir> <kelas_kereta>\n\
...
while perintah != "EXIT": 
    ...
    perintah_valid = ["INFO_TUJUAN", "TUJUAN_KELAS", "TUJUAN_JAM","TUJUAN_KELAS_TERMURAH", "TUJUAN_JAM_TERMURAH", "EXIT"] 
    ...
    # Bagian Bonus (TUJUAN_KELAS_TERMURAH)
    elif jenis_perintah == "TUJUAN_KELAS_TERMURAH":
        perintah = perintah.split()
        if len(perintah) <= 1:
            print("Perintah yang dimasukkan tidak valid")
        else:
            ...
            if kelas not in kelas_valid:
                ...
                for jadwal in daftar_jadwal:
                    if jadwal["tujuan_akhir"] == tujuan and jadwal["kelas"] == kelas:
                        ...
                        jam_berangkat = jadwal["jam_keberangkatan"]
                        harga_tiket = jadwal["harga_tiket"]
                        if harga_tiket < harga_termurah:
                            harga_termurah = harga_tiket
                            hasil = f"KA {nomor_ka} berangkat pukul {jam_berangkat} dengan harga tiket {harga_tiket}"
                            print(hasil)
                        elif harga_tiket == harga_termurah:
                            hasil += f"KA {nomor_ka} berangkat pukul {jam_berangkat} dengan harga tiket {harga_tiket}"
                            print(hasil)
                if not_found:
                    print("Tidak ada jadwal KA yang sesuai")
    ...
    # Bagian Bonus (TUJUAN_JAM_TERMURAH)
    elif jenis_perintah == "TUJUAN_JAM_TERMURAH":
        perintah = perintah.split()
        if len(perintah) <= 1:
            print("Perintah yang dimasukkan tidak valid")
        else:
